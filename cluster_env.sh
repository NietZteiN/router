...
# ── Interpreters ─────────────────────────────────────────────────────────────────────────────
# Three of them, because the tracks genuinely need three:
#   TOFU_PYTHON       requirements.txt      (torch 2.5.1 / transformers 4.48.3) — the main tree
#   TOFU_OU_PYTHON    requirements-ou.txt   (torch 2.4.1 / transformers 4.51.3) — open-unlearning
#                     evals for the sepmlp/blocktc/memadapt OU tracks. NOT interchangeable:
#                     the two transformers majors resolve the chat template differently.
#   TOFU_PLOT_PYTHON  needs matplotlib, which requirements.txt omits on purpose.
export TOFU_OU_PYTHON="${TOFU_OU_PYTHON:-${TOFU_PYTHON}}"
export TOFU_PLOT_PYTHON="${TOFU_PLOT_PYTHON:-${TOFU_PYTHON}}"
export TOFU_JUPYTER="${TOFU_JUPYTER:-jupyter}"

# ── Storage roots ────────────────────────────────────────────────────────────────────────────
# Before the export, 145 code files and 212 configs named the developer's scratch filesystem
# outright. They now reference these variables instead; repo_env.expand_paths resolves them at
# config-load time and hard-errors on an unset one.
#   TOFU_CKPT_ROOT   this repo's own store, set by the site file (was <scratch>/checkpoints/tofu_sisa_lora)
#   TOFU_CKPT_STORE  the PARENT of every project's store, so "${TOFU_CKPT_STORE}/ramole" reaches
#                    a sibling project's checkpoints    (was <scratch>/checkpoints)
#   TOFU_DATA_ROOT   non-HF datasets: counterfact, lume, pistol  (was <scratch>/data)
#   TOFU_STORAGE_ROOT the scratch filesystem itself      (was <scratch>)
# Derived, not duplicated: a site that keeps the original layout sets TOFU_CKPT_ROOT and gets the
# other three for free. A site that splits them across filesystems overrides each explicitly.
...
# ── GPU concurrency ceiling — CLAUDE.md §1, enforced here rather than trusted ─────────────────
# The rule is a GLOBAL cap of 4 concurrent GPUs summed across every queued job, not per-job.
# Each project's slurm_nodes.sh derives its own %N array throttle from this. Clamping it in one
# place means a site file (or a stray export) cannot quietly raise it: the ceiling is a property
# of the repo, and a would-be violation is reported instead of silently honoured.
