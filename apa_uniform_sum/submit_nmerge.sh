...
# HF_HOME comes from cluster_env.<site>.sh (sourced above, which hard-errors if the site file
# does not set it). The old `${HF_HOME:-${TOFU_CKPT_ROOT}/...}` default here was a second source of
# truth pointing at one specific cluster's disk.
...
# expandvars, exactly as merge_subset.load_config and submit_expa.sh do. Without it a config
# written as "${TOFU_CKPT_ROOT}/..." (the portable form) yields a LITERAL path here, and the
# driver then reports a missing manifest at a directory name containing '${TOFU_CKPT_ROOT}'.
print(os.path.expandvars(cfg["out_dir"]), cfg["model_name"], cfg["eval"]["k"],
      cfg["eval"]["forget_shard_id"], cfg["eval"]["cap"])
EOF
)"
[ -n "${OUT_DIR}" ] && case "${OUT_DIR}" in
  *'${'*) echo "out_dir still contains an unexpanded variable: ${OUT_DIR}" >&2
          echo "  the site file must export it — check cluster_env.\${TOFU_SITE}.sh" >&2; exit 1 ;;
esac
...
#SBATCH --array=${spec}
$(tofu_sbatch_resources 0 32 "${MERGE_MEM}")
...
# Under STUB the target filesystem may not exist — that is the point of previewing another
# site's scripts from here — so a failed mkdir must not abort the preview.
if ! mkdir -p "${LOG_DIR}" "${RESULTS_DIR}" 2>/dev/null; then
  [ "${STUB:-0}" = "1" ] || { echo "cannot create ${LOG_DIR} / ${RESULTS_DIR}"; exit 1; }
  echo "[stub] ${OUT_DIR} not present on this machine — preview only" >&2
fi
case "${STAGE}" in
  plan)    do_plan ;;
  merge)   do_merge ;;
