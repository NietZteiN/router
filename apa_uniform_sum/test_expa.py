...
# Pull HF_HOME *and* TOFU_CKPT_ROOT from cluster_env.<site>.sh. The checkpoint root matters
# here: the on-disk-manifest regression below is skipped when it is unset, and a silently
# skipped assertion is worse than a missing one.
...
def test_plan_is_identity_without_new_keys():
    ...
    # Regression reference: the on-disk manifest of the ORIGINAL additive_mean ladder. Present
    # only on a machine that has actually run it, so this half of the test is opportunistic —
    # the byte-identity assertions above it are the portable part and always run.
    ref = os.path.join(os.environ.get("TOFU_CKPT_ROOT", ""),
                       "Llama-2-7B-chat-hf_nmerge_r32") if os.environ.get("TOFU_CKPT_ROOT") else ""
    if ref and os.path.exists(f"{ref}/merge_manifest.txt"):
