...
# with the BlockTc eval integration: 2 files + 2 registration lines. Never
...
#
# DEFERRED (P4): written now, EXECUTED only at eval time with the user in the
# loop — the OU tree is dirty with the deliberate fp32-logits fix (guard 2).
#
...
cp "${HERE}/blocktc_registry.py" src/model/blocktc_registry.py
cp "${HERE}/BlockTc-Llama-3.2-1B.yaml" configs/model/BlockTc-Llama-3.2-1B.yaml

if ! grep -q "blocktc_registry" src/model/__init__.py; then
  cat >> src/model/__init__.py <<'EOF'

from model.blocktc_registry import BlockTcLlamaForCausalLM  # noqa: E402

_register_model(BlockTcLlamaForCausalLM)
EOF
fi

echo "BlockTc eval integration installed on branch memadapt-eval (NOT committed):"
git status --short -- src/model/blocktc_registry.py src/model/__init__.py \
                      configs/model/BlockTc-Llama-3.2-1B.yaml
cat <<'MSG'
Next step — only with the user's explicit approval — commit the additions:
  cd ${REPO_ROOT}/open-unlearning
  git add src/model/blocktc_registry.py src/model/__init__.py \
          configs/model/BlockTc-Llama-3.2-1B.yaml
  git commit -m "Add BlockTc eval integration (registry shim + model config)"
MSG
