...
- confidence-AUC half — PENDING (no enc roc JSONs — run analyze_router_leak.py roc on the rl_enc_*.sims.npz sidecars). Bar: per-encoder max confidence AUC (global_top1/per_expert/margin/knn_density; tomb_* excluded): CONFIRM <= 0.75, REFUTE >= 0.90, else inconclusive.
