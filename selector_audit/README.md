...
| [`eval_plain_ft.py`](eval_plain_ft.py) | Serves ONE model (no router, no pool) over the audit's own query conditions and dumps generations in the shape `csar.classify` already consumes. Rows, transforms, attacker and seed are imported from `analyze_router_shift`, never reimplemented, so the baseline is measured on exactly the queries findings 4 and 5 were. `--row_shard i/N` for GPU fan-out. |
| [`report_plain_ft.py`](report_plain_ft.py) | Stitches the row-shards back together and writes the two comparison tables. Refuses to score an arm with a HOLE in its shard set, and keeps the routed system's routing-capture rate in its own column — finding 5's 97.7% is a routing criterion and a routerless model has no route, so the shared criterion is what the answer SAYS. |
| [`test_plain_ft.py`](test_plain_ft.py) | CPU gate for the three ways that harness could produce a plausible wrong number: scoring an arm with a hole in its shard set, crediting the base model's own knowledge to the injection attack, and calling a row anonymised when stripping left a surname fragment. |
| [`dump_anonymized_examples.py`](dump_anonymized_examples.py) | Twenty (original, name-stripped, gold) triples for judging whether anonymised questions are still answerable in principle. A viewer, not a new transform. Marks the rows where stripping is a no-op instead of quietly sampling around them. |
