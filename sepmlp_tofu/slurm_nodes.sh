# SLURM environment for sepmlp_tofu — source from submit scripts.
...
# NEVER raise SEPMLP_CAP above 4.
...
export SEPMLP_EXCLUDE="${SEPMLP_EXCLUDE:-${TOFU_EXCLUDE:-}}"
export SEPMLP_CAP="${SEPMLP_CAP:-${TOFU_ARRAY_CAP:-4}}"                # global cap (do not raise)
export SEPMLP_THROTTLE="${SEPMLP_THROTTLE:-2}"      # per-array %N throttle
export SEPMLP_MEM="${SEPMLP_MEM:-48G}"
export SEPMLP_CPUS="${SEPMLP_CPUS:-8}"

export SEPMLP_SMOKE_TIME="${SEPMLP_SMOKE_TIME:-00:40:00}"
export SEPMLP_PILOT_TIME="${SEPMLP_PILOT_TIME:-01:30:00}"
export SEPMLP_TRAIN_TIME="${SEPMLP_TRAIN_TIME:-04:00:00}"
export SEPMLP_PROBE_TIME="${SEPMLP_PROBE_TIME:-01:00:00}"
export SEPMLP_EVAL_TIME="${SEPMLP_EVAL_TIME:-03:00:00}"
export SEPMLP_RELEARN_TIME="${SEPMLP_RELEARN_TIME:-01:00:00}"
...
export SEPMLP_ROOT="${SEPMLP_ROOT:-${TOFU_CKPT_STORE}/sepmlp_tofu}"
...
# (populated online on the login node, 2026-07-15; shared with memadapt).
